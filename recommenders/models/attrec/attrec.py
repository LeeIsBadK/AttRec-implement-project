...
class AttRec:
    def __init__(self, config):
        ...
        self.msl = config.sequence_length #5
        self.tsl = config.target_length #3
    ...
    def build_model(self):
        with tf.variable_scope('AttRec'):
            ...
            # s = tf.reduce_sum(output, axis=1)  #[B, e]
            max_val = tf.reduce_max(output, axis=1)  #[B, e]
            # min_val = tf.reduce_min(output, axis=1)  #[B, e]
            ...
            self.pos_y = pos_y = self.pos_object_function(u, pos_v, m, pos_x, self.w) #[B,tsl] [256,3]
            self.neg_y = neg_y = self.neg_object_function(u, neg_v, m, neg_x, self.w) #[B,nsl] [256,10]

            #margin based hinge loss
            self.loss = self.loss_function(self.gamma, pos_y, neg_y)
            self.next_items = max_val
    ...
    def attention_module(self,query,key,value,unit):
        with tf.variable_scope('attention',reuse=True):
            relu_q = tf.keras.layers.Dense(unit, name='qk_map', activation=tf.nn.relu, use_bias=False,kernel_initializer=self.u_init,
                                    kernel_regularizer=tf.keras.regularizers.l2(0.003))
            query = relu_q(query)  # Apply the layer to the input tensor
            query = tf.nn.dropout(query, self.keep_prob)

            relu_k = tf.keras.layers.Dense(self.emb_size, name='qk_map', activation=tf.nn.relu, use_bias=False,kernel_initializer=self.u_init,
                                  kernel_regularizer=tf.keras.regularizers.l2(0.003))
            key = relu_k(key)
    ...
    def TE(self, t, i, d): # Time embedding
        if i % 2 == 0:
